...
if [ -f "/rocker_scripts/.s6_version" ] && [ "$S6_VERSION" = "$(cat /rocker_scripts/.s6_version)" ]; then
  ...
  wget -P /tmp/ http://172.17.0.1:9000/github.com/just-containers/s6-overlay/releases/download/${S6_VERSION}/s6-overlay-aarch64.tar.gz

  ## need the modified double tar now, see https://github.com/just-containers/s6-overlay/issues/288
  tar hzxf /tmp/s6-overlay-aarch64.tar.gz -C / --exclude=usr/bin/execlineb
  tar hzxf /tmp/s6-overlay-aarch64.tar.gz -C /usr ./bin/execlineb && $_clean
